        ...
        stage('Build-image') {
            steps {
                sh ''' #!/bin/bash
                  ...
                  docker tag chatapp:latest manidevasani/chatapp:$BUILD_NUMBER
         ...
         stage('Build-Container') {
            steps {
                sh ''' #!/bin/bash           
                  ...
                  docker pull manidevasani/chatapp:$BUILD_NUMBER
                  docker run --name chatproject  --network ubuntu_network -d -p 80:8000 manidevasani/chatapp:$BUILD_NUMBER
                  docker restart db
